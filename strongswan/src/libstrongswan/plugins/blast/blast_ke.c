 ...
 */
METHOD(key_exchange_t, get_public_key, bool, private_blast_ke_t *this,
       chunk_t *value) {

  symmetric_key_data_t key_data;
  key_config_t key_config = {lib->settings->get_int(
      lib->settings, "%s.plugins.blast.ttl", 0, lib->ns)};

  DBG2(DBG_LIB, "[BLAST] Enter %s, %s (%d)", __func__, __FILE__, __LINE__);

  if (this->endpoint_type == EP_TYPE_INITIATOR) {

    DBG1(DBG_LIB, "[BLAST] Error: unexpected endpoint_type %d",
         this->endpoint_type);
    return FALSE;

  } else if (this->endpoint_type == EP_TYPE_UNKNOWN) {

    this->endpoint_type = EP_TYPE_INITIATOR; // get_public_key is the first KE
                                             // call by the initiator

    // Perform blast genInit
    DBG2(DBG_LIB, "[BLAST] (ALICE) %s: calling qrypt_security_gen_init_aes...",
         __func__);
    int ret_code = qrypt_security_gen_init_aes(&this->qrypt_security, &key_data,
                                               key_config);
    if (ret_code != QS_GOOD) {
      DBG1(DBG_LIB, "[BLAST] Error: qrypt_security_gen_init_aes returned %s",
           qs_error_str(ret_code));
      return FALSE;
    }
    ...
    this->shared_secret =
        chunk_clone(chunk_create(key_data.key, key_data.key_size));

    DBG2(DBG_LIB, "[BLAST] (ALICE) %s: returning metadata...", __func__);
    *value =
        chunk_clone(chunk_create(key_data.metadata, key_data.metadata_size));

    // Free key data structure
    ret_code = qrypt_security_symmetric_key_data_free(&key_data);
    if (ret_code != QS_GOOD) {
      DBG1(DBG_LIB,
           "[BLAST] Error: qrypt_security_symmetric_key_data_free returned %s",
           qs_error_str(ret_code));
      return FALSE;
    }
 ...
 */
METHOD(key_exchange_t, set_public_key, bool, private_blast_ke_t *this,
       chunk_t value) {
  ...
  if (this->endpoint_type == EP_TYPE_RESPONDER) {
    ...
    this->endpoint_type = EP_TYPE_RESPONDER; // set_public_key is the first KE
                                             // call by the responder
    ...
    DBG1(DBG_LIB,
         "[BLAST] (BOB) Peer's public key successfully verified and set!");
 ...
 */
METHOD(key_exchange_t, get_shared_secret, bool, private_blast_ke_t *this,
       chunk_t *secret) {
  ...
  if (this->endpoint_type == EP_TYPE_RESPONDER) {
    ...
    DBG2(DBG_LIB, "[BLAST] (BOB) %s: calling qrypt_security_gen_sync...",
         __func__);
    int ret_code = qrypt_security_gen_sync(
        &this->qrypt_security, &key_data); // TODO: Update c wrapper to split
                                           // key_data to metadata and key
    if (ret_code != QS_GOOD) {
      DBG1(DBG_LIB, "[BLAST] Error: qrypt_security_gen_sync returned %s",
           qs_error_str(ret_code));
      return FALSE;
    }
...
METHOD(key_exchange_t, destroy, void, private_blast_ke_t *this) {
  ...
  if (ret_code != QS_GOOD) {
    DBG1(DBG_LIB, "[BLAST] Error: qrypt_security_delete returned %s",
         qs_error_str(ret_code));
    return;
  }
...
blast_ke_t *blast_ke_create(key_exchange_method_t method) {
  ...
  token = lib->settings->get_str(lib->settings, "%s.plugins.blast.token", NULL,
                                 lib->ns);
  if (token == NULL) {
  ...
  INIT(this,
       ...
       .method = method, .endpoint_type = EP_TYPE_UNKNOWN,
       .metadata = chunk_empty, .shared_secret = chunk_empty, );

  int ret_code = qrypt_security_create(&this->qrypt_security);
  if (ret_code != QS_GOOD) {
    DBG1(DBG_LIB, "[BLAST] Error: qrypt_security_create returned %s",
         qs_error_str(ret_code));
    free(this);
    return NULL;
  }
  ...
  char *servers_str = lib->settings->get_str(
      lib->settings, "%s.plugins.blast.servers", NULL, lib->ns);
  ...
  if (servers_str != NULL) {
    ...
    DBG2(DBG_LIB,
         "[BLAST] no servers configured, will query directory service");
  }

  /* Read auth header type (omitted/zero = BEARER_AUTH, the SDK default) */
  enum qrypt_auth_header_type auth_type;
  char *auth_str = lib->settings->get_str(
      lib->settings, "%s.plugins.blast.auth_header_type", NULL, lib->ns);
  ...
  char *ca_cert_path = lib->settings->get_str(
      lib->settings, "%s.plugins.blast.ca_cert_path",
      "/etc/ssl/certs/ca-certificates.crt", lib->ns);
  ...
  client_config_t client_config = {
      ...
      .static_servers_tg = lib->settings->get_int(
          lib->settings, "%s.plugins.blast.tg", 0, lib->ns),
      .static_servers_tf = lib->settings->get_int(
          lib->settings, "%s.plugins.blast.tf", 0, lib->ns),
      .static_servers_ta = lib->settings->get_int(
          lib->settings, "%s.plugins.blast.ta", 0, lib->ns),
      .static_servers_tp = lib->settings->get_int(
          lib->settings, "%s.plugins.blast.tp", 0, lib->ns),
      .auth_header_type = auth_type,
      .user_agent = user_agent,
  };

  ret_code = qrypt_security_initialize_client_config(
      &this->qrypt_security, token, token_length, client_config);
  if (ret_code != QS_GOOD) {
    DBG1(DBG_LIB, "[BLAST] Error: qrypt_security_initialize returned %s",
         qs_error_str(ret_code));
    free(servers_buf);
